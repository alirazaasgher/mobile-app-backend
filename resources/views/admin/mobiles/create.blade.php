  ...
  <form action="{{ isset($mobile) ? route('mobiles.update', $mobile->id) : route('mobiles.store') }}" method="POST"
    ...
    <div class="space-y-4 mb-6">
      ...
      <div>
        ...
        <div id="{{ $descEditorId }}" class="quill-editor bg-white min-h-[200px]"
          data-target="{{ $descTextareaId }}"></div>

        <textarea name="description" id="{{ $descTextareaId }}" class="hidden">{!! $mobile->description !!}</textarea>
      </div>
      <div class="grid grid-cols-2 gap-4">
  <div class="mb-4">
    <label class="block font-medium mb-2">Pros</label>
    ...
    <div id="{{ $prosEditorId }}" class="quill-editor bg-white min-h-[120px]"
      data-target="{{ $prosTextareaId }}"></div>

    <textarea name="pros" id="{{ $prosTextareaId }}" class="hidden">{!! old('pros', $mobile->pros ?? '') !!}</textarea>
  </div>
  
  <div class="mb-4">
    <label class="block font-medium mb-2">Cons</label>
    ...
    <div id="{{ $consEditorId }}" class="quill-editor bg-white min-h-[120px]"
      data-target="{{ $consTextareaId }}"></div>

    <textarea name="cons" id="{{ $consTextareaId }}" class="hidden">{!! old('cons', $mobile->cons ?? '') !!}</textarea>
  </div>
</div>
